...
exports.addObject = async function addObject(req, res, next) {
  ...
  try {
    ...
    switch (e.status) {
      case 400:
        res.status(e.status);
        res.json(e.message);
        fs.appendFile(logOperations.logFilePath, `${date} - Status Code: ${e.status} - ${e.message} at Upload controller - Host: ${req.headers.host} IP: ${req.ip}\n`, (err) => {
